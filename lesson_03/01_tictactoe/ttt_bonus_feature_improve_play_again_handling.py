# rules:
    # Only accept inputs of y, Y, n, or N
    # Issue error message if not y, Y, n, or N
# algorithm:
    # Ask if user wants to play again and validate user input
        # Prompt message and get input
        # While the input is invalid, issue error and get input
            # If input isn't in a list with y, Y, n, or N, input is invalid
            # Else input is valid
    # If yes, return True
    # If no, return False
...
def get_who_first():
    ...
    if WHO_FIRST == 'choose':
        while True:
            ...
            while True:
                user_choice = input().strip().lower()  # added strip
                if user_choice in ['player', 'computer']:
                    return user_choice
                else:
                    prompt('Invalid input. Select who starts first: '
                    '"Player" or "Computer".')  # reworded error message
